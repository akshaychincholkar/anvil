...
// Seed: past days with a mood logged (day → mood id)
const SEED_HISTORY = { 1:4, 2:5, 3:3, 4:4, 5:2, 6:4, 7:5, 8:3, 9:4, 10:5, 11:4, 12:3 };
...
export default function JournalScreen() {
  ...
  const addBullet = (sid) => {
    ...
    if (!text) return;
    setEntries((e) => ({ ...e, [sid]: [...e[sid], text] }));
    setDrafts((d) => ({ ...d, [sid]: "" }));
  };
  const removeBullet = (sid, i) =>
    setEntries((e) => ({ ...e, [sid]: e[sid].filter((_, idx) => idx !== i) }));

  // streak: consecutive logged days back from yesterday + today-if-logged
  const loggedToday = mood !== null;
  const streak = (() => {
    let s = loggedToday ? 1 : 0;
    for (let d = TODAY - 1; d >= 1; d--) { if (history[d]) s++; else break; }
    return s;
  })();

  const totalBullets = Object.values(entries).reduce((n, arr) => n + arr.length, 0);

  return (
    <div style={S.page}>
      <div style={S.head}>
        <div>
          <div style={S.eyebrow}>BRTN · Journal</div>
          <h1 style={S.h1}>Today</h1>
          <div style={S.date}>Saturday, 13 June 2026</div>
        </div>
      ...
      {/* ── MOOD CALENDAR ────────────────────────── */}
      <div style={S.card}>
        <div style={S.cardTitle}>{MONTH_LABEL} · Mood history</div>
        <div style={S.calDow}>
          {DOW.map((d) => <div key={d} style={S.calDowCell}>{d}</div>)}
        </div>
        <div style={S.calGrid}>
          {Array.from({ length: DAYS_IN_MONTH }, (_, i) => i + 1).map((day) => {
            const m = day === TODAY && mood ? moodById(mood) : (history[day] ? moodById(history[day]) : null);
...
const S = {
  ...
  card: { background: "#fff", borderRadius: 12, padding: "16px 16px", marginBottom: 12, boxShadow: "0 1px 3px rgba(0,0,0,0.05)" },
